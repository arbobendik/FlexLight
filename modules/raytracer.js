...
export class RayTracer {
  ...
  vec2 reservoirSample (sampler2D light_tex, Ray ray, vec3 random_vec, vec3 last_rough_normal, vec3 last_origin, vec3 last_rme, bool dont_filter, int i) {
    ...
    for (int j = 0; j < textureSize(light_tex, 0).y; j++) {
      ...
      if (last_random.y * (weight + total_weight) <= weight) {
    ...
    if (reservoir_length == 0.0) return vec2(0);

    vec2 result = vec2(0);
    // Test if in shadow
    if (!shadowTest(reservoir_light_ray, reservoir_light)) {
      result.x += reservoir_weight;
    } else if (dont_filter || i == 0) {
      result.y += pow(2.0, - float(i + 2));
    }
    // Weight result.
    result.x *= total_weight / (reservoir_length * reservoir_weight);
  ...
  vec3 lightTrace(sampler2D world_tex, sampler2D light_tex, vec3 origin, Ray first_ray, vec3 rme, vec3 tpo, int sample_n, int bounces){
    ...
    for (int i = 0; i < bounces && length(importancy_factor) >= min_importancy * SQRT3; i++){
      ...
      if (is_solid) {
        ...
        // vec2 fels = reservoirSample (light_tex, ray, random_vec, last_rough_normal, last_origin, last_rme, dont_filter, i);
  ...
  async render() {
    ...
    let resize = () => {
      ...
      rt.firstPasses = 1;
      rt.secondPasses = Math.max(2 + Math.round(Math.min(canvas.width, canvas.height) / 800), 2);
    }
    ...
    function renderTextureBuilder(){
      for (let i = 0; i < rt.temporalSamples; i++) {
        ...
        GLLib.randomTextureBuilder(rt.#gl.canvas.width / 8, rt.#gl.canvas.height / 8, rt.#gl, rt.#randomTextures[i]);
      }
