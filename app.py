...
@app.websocket('/ws')
async def websocket_endpoint(s: WebSocket):
...
async def vector(pkg, s):
    ...
    p = this_game.identify_player(this_database, ip, pkg['keys'])
    await connections.broadcast('player_vector_change', {'player': p.keys['public'], 'dx': pkg['dx'], 'dy': pkg['dy']}, s)
...
async def sync_game(pkg, s, **kwargs):
    ...
    await connections.emit('sync_game', {'players': [op.jsonify() for keys, op in this_game.active_players.items() if not op == p]}, s)

# Initialize api global to handel socket communication.
api = {
    'init': init,
    'vector': vector,
    'sync_game': sync_game
