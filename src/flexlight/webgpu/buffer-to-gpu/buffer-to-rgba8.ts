...
export type TypedArrayR8 = Uint8Array | Int8Array;
export type TypedArrayR8Type = "uint" | "sint";
const TEXTURE_SIZE_2D: Vector<2> = new Vector(2048, 2048);


export class BufferToRGBA8<T extends TypedArrayR8> extends BufferToGPU {
    ...
    private type: TypedArrayR8Type;
    private label: string;

    constructor(bufferManager: BufferManager<T>, device: GPUDevice, type: TypedArrayR8Type = "uint", label: string = "") {
        super();
        ...
        this._gpuTexture = this.device.createTexture({
            ...
            format: ("rgba8" + type) as GPUTextureFormat,
    ...
    reconstruct = () => {
        ...
        if (4 * this.gpuTextureSize.x * this.gpuTextureSize.y * this.gpuTextureSize.z <= this.bufferManager.length) {
            ...
            this._gpuTexture = this.device.createTexture({
                // dimension: "3d",
                size: this.gpuTextureSize,
                format: ("rgba8" + this.type) as GPUTextureFormat,
                usage: GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.STORAGE_BINDING | GPUTextureUsage.COPY_DST,
                label: this.label
            });
