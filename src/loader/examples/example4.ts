...
let statue = await loadObj('woman');
...
const dragon_instance = scene.instance(dragon);
...
const statue_instance = scene.instance(statue);
statue_instance.transform.position = new Vector(13, -4, -8);
statue_instance.transform.rotateAxis(new Vector(0, 1, 0), Math.PI / 4);
statue_instance.transform.scale(2.5);
// statue_instance.material.color = new Vector(0.95, 0.5, 0.3);
...
statue_instance.material.ior = 1.4;

const robot_instance = scene.instance(robot);
